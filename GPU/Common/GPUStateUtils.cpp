...
void ConvertViewportAndScissor(bool useBufferedRendering, float renderWidth, float renderHeight, int bufferWidth, int bufferHeight, ViewportAndScissor &out) {
	...
	if (throughmode) {
		...
		float nearZ = (inverted ? depthRangeMax : depthRangeMin) * (1.0f / 65535.0f);
		float farZ = (inverted ? depthRangeMin : depthRangeMax) * (1.0f / 65535.0f);

		if (nearZ < 0.0f || farZ > 1.0f) {
			float overageNear = std::max(-nearZ, 0.0f);
			float overageFar = std::max(farZ - 1.0f, 0.0f);
			float drift = overageFar - overageNear;

			nearZ += overageNear;
			farZ -= overageFar;

			zScale = fabsf(vpZScale * (2.0f / 65535.0f)) / (farZ - nearZ);
			zOffset = drift / (farZ - nearZ);
		}
		...
		out.depthRangeMin = inverted ? farZ : nearZ;
		out.depthRangeMax = inverted ? nearZ : farZ;
...
static void ConvertStencilFunc5551(GenericStencilFuncState &state) {
	...
	auto rewriteFunc = [&](GEComparison func, u8 ref, u8 mask) {
